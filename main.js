...
let toggleBtn = document.querySelector('#toggle input[type="checkbox"]');
    let storageKey = 'toggle-state';

    // Retrieve the state of the toggle button from local storage
    ...
    // Update the local storage when the toggle button is clicked
    toggleBtn.addEventListener('click', () => {
      let currentState = toggleBtn.checked;
      localStorage.setItem(storageKey, currentState.toString());
      if (currentState) {
        document.body.classList.add('light-mode');
      } else {
